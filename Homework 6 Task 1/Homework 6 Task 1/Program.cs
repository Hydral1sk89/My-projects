...
namespace Homework_6_Task_1
{
    class Program
    {
        ...
        static public void hakaton2(int[][] numbers)
        {
            int n = numbers[0].Length;

            using (StreamWriter sw = new StreamWriter("hakaton.csv", true, Encoding.Unicode))
            {
                ...
                } while (n >= 0); // до тех пор пока не закончится массив
            }
        }

        static public void StreamReadWrite()
        {
